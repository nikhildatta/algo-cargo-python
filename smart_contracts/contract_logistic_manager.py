...
class LogisticManagerContract:
    ...
    class Variables:
        ...
        delivery_unit_cost = Bytes("delivery_unit_cost")  # Int
        app_state = Bytes("delivery_state")  # Int
        delivery_capacity = Bytes("delivery_capacity")  # Int
    ...
    class AppMethods:
        ...
        participate_delivery = "participateDelivery"
        start_delivery = "startDelivery"
        finish_delivery = "finishDelivery"
        cancel_delivery_participation = "cancelParticipation"
    ...
    def application_start(self):
        ...
        handle_noop = Seq(
            Cond(
                ...
                [Txn.application_args[0] == Bytes(self.AppMethods.update_delivery),
                 self.update_delivery()],

                [Txn.application_args[0] == Bytes(self.AppMethods.participate_delivery),
                 self.participate_delivery()],

                [Txn.application_args[0] == Bytes(self.AppMethods.cancel_delivery_participation),
                 self.cancel_participation()],

                [Txn.application_args[0] == Bytes(self.AppMethods.start_delivery),
                 self.start_delivery()],

                [Txn.application_args[0] == Bytes(self.AppMethods.finish_delivery),
                 self.finish_delivery()]
            )
        )

        no_participants = App.globalGet(self.Variables.delivery_capacity) == App.globalGet(
            self.Variables.max_capacity)

        delivery_started = App.globalGet(self.Variables.app_state) == self.AppState.started
        delivery_finished = App.globalGet(self.Variables.app_state) == self.AppState.finished
        ...
        can_delete = And(
            is_creator,
            Or(no_participants, delivery_finished)
        )
    ...
    def app_create(self):
        ...
        return Seq([
            ...
            App.globalPut(self.Variables.delivery_unit_cost, Btoi(Txn.application_args[7])),
            App.globalPut(self.Variables.max_capacity, Btoi(Txn.application_args[8])),
            App.globalPut(self.Variables.delivery_capacity, Btoi(Txn.application_args[8])),
    ...
    def update_delivery(self):
        ...
        no_participants = App.globalGet(self.Variables.delivery_capacity) == App.globalGet(self.Variables.max_capacity)
        delivery_ready = App.globalGet(self.Variables.app_state) == self.AppState.ready
        is_creator = Txn.sender() == App.globalGet(self.Variables.creator_address)
        ...
        return Seq([
            ...
            App.globalPut(self.Variables.delivery_unit_cost, Btoi(Txn.application_args[8])),
            App.globalPut(self.Variables.max_capacity, Btoi(Txn.application_args[9])),
            App.globalPut(self.Variables.delivery_capacity, Btoi(Txn.application_args[9])),
            ...
            Assert(App.globalGet(self.Variables.max_capacity) > Int(0)),
            Return(Int(1))
        ])
    ...
    def initialize_escrow(self, escrow_address):
        ...
        is_creator = Txn.sender() == App.globalGet(self.Variables.creator_address)
        delivery_not_init = App.globalGet(self.Variables.app_state) == self.AppState.not_initialized
    ...
    def fund_escrow(self):
        ...
        is_creator = Txn.sender() == App.globalGet(self.Variables.creator_address)
        delivery_init = App.globalGet(self.Variables.app_state) == self.AppState.initialized
    ...
    def opt_in(self):
        ...
        return Seq([
            ...
            Assert(Global.round() <= App.globalGet(self.Variables.departure_date_round)),
            Assert(App.globalGet(self.Variables.delivery_capacity) > Int(0)),
            Return(Int(1))
        ])

    def participate_delivery(self):
        """
        NoOpTxn
        A user want to participate the delivery
        Perform validity checks and payment checks
        :return:
        """
        get_participant_state = App.localGetEx(Int(0), App.id(), self.Variables.book_capacity)
        delivery_capacity = App.globalGet(self.Variables.delivery_capacity)
        valid_number_of_transactions = Global.group_size() == Int(2)
        is_creator = Txn.sender() == App.globalGet(self.Variables.creator_address)
        ...
        can_participate = And(
            ...
            Not(is_creator),
            App.globalGet(self.Variables.delivery_capacity) >= book_capacity,  # check if there is an available seat
            Global.round() <= App.globalGet(self.Variables.departure_date_round),  # check if delivery is started
        ...
        valid_payment = And(
            Gtxn[1].type_enum() == TxnType.Payment,
            Gtxn[1].receiver() == App.globalGet(self.Variables.escrow_address),
            Gtxn[1].amount() == App.globalGet(self.Variables.delivery_unit_cost)*book_capacity,
            Gtxn[1].sender() == Gtxn[0].sender(),
        )
        ...
        update_state = Seq([
            ...
            # update state
            App.globalPut(self.Variables.delivery_capacity, delivery_capacity - book_capacity),  # decrease seats
    ...
    def cancel_participation(self):
        """
        NoOpTxn
        A user want to cancel delivery participation
        ...
        valid_refund = And(
            ...
            Gtxn[1].amount() == App.globalGet(self.Variables.delivery_unit_cost)*App.localGet(Int(0), self.Variables.book_capacity),
        ...
        update_state = Seq([
            ...
            # update state
            App.globalPut(self.Variables.delivery_capacity, delivery_capacity + App.localGet(Int(0), self.Variables.book_capacity)),  # increase seats
    ...
    def start_delivery(self):
        """
        NoOpTxn
        The creator start the delivery
        Perform validity checks and payment checks
        :return:
        """
        is_creator = Txn.sender() == App.globalGet(self.Variables.creator_address)
    ...
    def finish_delivery(self):
        """
        NoOpTxn
        The creator finish the delivery
        Perform validity checks and payment checks
        :return:
        """
        is_creator = Txn.sender() == App.globalGet(self.Variables.creator_address)
        valid_number_of_transactions = Global.group_size() == Int(2)
        amount = App.globalGet(self.Variables.max_capacity) - App.globalGet(self.Variables.delivery_capacity)
        amount = amount * App.globalGet(self.Variables.delivery_unit_cost)

        can_finish = And(
            App.globalGet(self.Variables.app_state) == self.AppState.started,
            is_creator,  # creator only can perform this action
            Global.round() <= App.globalGet(self.Variables.arrival_date_round),
            valid_number_of_transactions
        )
